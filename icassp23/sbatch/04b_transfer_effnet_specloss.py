...
for init_id in range(n_inits):
    ...
    model_path = "efficientnet_ploss_None_10_12_batch_size64_bn_var0.5_init-{}_LMA_0.0_constant_brake_1.00_damping_mean_outdim-5_log-True_minmax-True_lr-0.001".format(init_id)
    ckpt_path = os.path.join(save_dir, "f_W", model_path, ckpt_path)
    # Generate file.
    with open(file_path, "w") as f:
        cmd_args = [script_path.replace("transfer", "train"), save_dir, str(init_id), str(batch_size), ckpt_path]
...
with open(file_path, "w") as f:
    # Print header.
    f.write(
        "# This shell script trains EfficientNet on MSS spectral loss."
    )
    f.write("\n")
