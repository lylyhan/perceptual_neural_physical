...
weight_dir = os.path.join(save_dir, "J_log")
...
if __name__ == "__main__":
    ...
    pretrainmodel_save_path = os.path.join(model_dir, 
                                           "ploss_finetuneFalse_log-1_minmax-1_opt-adam_batch_size64_lr-0.001_init-0")
    ...
    # train
    ckpt_path = os.path.join(pretrainmodel_save_path, 'ckpt-epoch=34-val_loss=0.04.ckpt')
    print("Load Pretrained model")
    model = model.load_from_checkpoint(
        os.path.join(pretrainmodel_save_path, ckpt_path),  
    ...
    test_loss = trainer_ft.test(model, dataset_mss, verbose=False)
