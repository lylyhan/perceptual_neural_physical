...
if len(sys.argv) < 5:
    ...
    is_train = False #change to false after running this
...
weight_dir = os.path.join(save_dir, "J_nominmax_log")
...
synth_type = "amchirp"
minmax = False
assert utils.logscale == logscale_theta == setups.logscale
...
if __name__ == "__main__":
    ...
    if cnn_type == "cnn.wav2shape":
        ...
        model = cnn.EffNet(in_channels=1, outdim=outdim, loss=loss_type, scaler=scaler, var=bn_var, LMA=LMA, minmax=False, save_path=pred_path)
    ...
    if is_train:
        ...
        model = model.load_from_checkpoint(os.path.join(model_save_path, ckpt_path),in_channels=1, outdim=outdim, loss=loss_type, scaler=scaler, var=bn_var, LMA=LMA, minmax=False, save_path=pred_path)
        trainer.fit(model, dataset) #keep training, change back
