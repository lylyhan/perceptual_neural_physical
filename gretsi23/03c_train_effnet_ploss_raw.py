"""
This script trains an EfficientNet for sound matching of am chirp sounds
...
weight_dir = os.path.join(save_dir, "J_nolog")
...
assert setups.logscale == logscale_theta
...
if __name__ == "__main__":
    ...
    if cnn_type == "cnn.wav2shape":
        ...
        model = cnn.EffNet(in_channels=1, outdim=outdim, loss=loss_type, scaler=scaler, var=bn_var, save_path=pred_path, minmax=False)
    ...
    if is_train:
        ...
        model = model.load_from_checkpoint(os.path.join(model_save_path, ckpt_path),in_channels=1, outdim=outdim, loss=loss_type, scaler=scaler, var=bn_var, save_path=pred_path, minmax=False)
