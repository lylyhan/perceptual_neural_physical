...
def evaluate_gradnorm(model, nbatch, loss_type):
    model.zero_grad()
...
def eval_smooth(prev_model, model, nbatch, loss_type,  num_pts=1):
    ...
    for alpha in alphas:
        ...
        evaluate_gradnorm(new_model, nbatch, loss_type)
...
model = cnn.EffNet(in_channels=1, outdim=outdim, loss="ploss", eff_type=eff_type, 
...
np.save("./{}_{}_gradnorms.npy".format(opt, "ploss_pnp"), gradnorms)
np.save("./{}_{}_smoothness.npy".format(opt, "ploss_pnp"), smooths)
